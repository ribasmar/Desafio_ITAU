...
```bash
# Caminho completo com a .venv do projeto
/home/gaspar/projects/ITAUQuant/Desafio_ITAU/.venv/bin/python src/copom/ingest/collect.py
```

Ou via módulo (precisa de `PYTHONPATH` se o pacote não está instalado):

```bash
PYTHONPATH=src /home/gaspar/projects/ITAUQuant/Desafio_ITAU/.venv/bin/python -m copom.ingest
```
