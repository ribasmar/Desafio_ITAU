# Copom Quant AI — Tom da Comunicação do Copom
...
| 2 | Extração de tom | `src/copom/features/` | LLM local determinístico → JSON auditável **+** baseline léxico |
...
## Extração de tom — LLM como função determinística

A LLM **não é o produto**; é uma função determinística (temperatura 0) que recebe texto e devolve números auditáveis. Saída estruturada:

```json
{
  "stance": 0.0,            // -1 (dovish) a +1 (hawkish)
  "stance_delta": 0.0,      // variação vs. comunicação anterior
  "forward_guidance": "neutro",  // {aperto, manutencao, afrouxamento, neutro}
  "incerteza": 0.0,         // 0 a 1
  "conviccao": 0.0,         // 0 a 1
  "justificativa": "..."    // trecho citado que embasa o score
}
```

Usamos um **modelo open-source local** (pesos fixados, *greedy decoding*): a banca pode rodar o código e obter **exatamente o mesmo JSON** — reprodutibilidade que uma API fechada não garante. **Sem fine-tuning** (amostra pequena demais). O prompt é versionado em `prompts/` e instrui o modelo a pontuar **apenas pelo texto fornecido**, sem inferir o desfecho (defesa contra contaminação por *hindsight*).
...
│   ├── features/     # camada 2: extract_tone (LLM local) + baseline léxico
...
├── prompts/          # copom_v1.md — prompt versionado do extrator
├── notebooks/        # EDA e resultados (consomem o pipeline, não o definem)
├── docs/             # realinhamento, decisões, sprint
└── tests/
```

## Stack

Python 3.11+ · `pandas` · `scikit-learn` · `statsmodels` · `httpx` (ingestão API BCB) · LLM local via [Ollama](https://ollama.com) (ou `transformers`) · `jupyter`. Deliberadamente enxuta: a prioridade é **correção e reprodutibilidade**, não sofisticação de stack — o rubrica não pontua complexidade.
...
Rafael Ribas · Gustavo More · Elder Nunes — Engenharia da Computação, UTFPR-Apucarana. Mentoria: Randerson Melville. Validação estatística: a confirmar.
